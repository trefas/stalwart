...
pub(crate) static ENTITIES: phf::Map<&'static str, u32> = phf::phf_map! {
    "AElig" => 198,
    "AMP" => 38,
    "Aacute" => 193,
    "Abreve" => 258,
...
fn main() {
    ...
    for entitiy in entities {
        println!("Looking up entity: {:?}", entitiy);
        assert_eq!(
            ENTITIES.get(entitiy).unwrap(),
            lookup_entity(entitiy.as_bytes()).unwrap()
        );
    }
    ...
    for word in words {
        ...
        assert_eq!(
            *WORDS.get(word).unwrap(),
            lookup_words(word.as_bytes()).unwrap()
        );
    }
